...
if "model_scores" in st.session_state and st.session_state["selected_tab"] == selected_tab:
    ...
    if metrics:
        ...
        name_map = {"rf":"Random Forest","xgb":"XGBoost","cat":"CatBoost","lr":"Logistic Regression","iso":"IsolationForest"}
        st.success(f"⭐ Best Model: **{name_map[best_key]}** — F1 Score: **{metrics[best_key]['f1']:.4f}**")
    ...
    if metrics:
        ...
        display_names = {"Random Forest":"rf","XGBoost":"xgb","CatBoost":"cat","Logistic Regression":"lr","IsolationForest":"iso"}
